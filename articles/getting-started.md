...
but you can override any of these:
...
summary then reports row and column counts, the engine used, and the
elapsed time, and the right panel previews the synthetic records.
...
The in-app download is the **compact** bundle: a small, shareable
package built around six files:
...
If a rendered comparison is included, you also get
`comparison_report.html`.

In the compact bundle, `README.md` is consolidated: it includes the
privacy summary and a **For AI assistants** section, so there is no
separate `ai-readme.md` or `privacy_report.txt`. Those standalone files
stay in the **full** CLI / agent bundle (`compact = FALSE` or
[`make_agent_bundle()`](https://lennon-li.github.io/dataganger/reference/make_agent_bundle.md)),
along with the extra diagnostics used in agent workflows.
